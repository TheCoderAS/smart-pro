/*
 * Unisync - Extension Firmware v4.2
...
static uint32_t crc32_compute(const uint8_t *data, uint8_t len) {
    ...
    /* Feed bytes -- CRC unit accepts 8/16/32-bit writes */
...
static void set_relay1(bool s) {
    relay1_state = s;
    if (s) { PIN_SET(PIN_RELAY1); PIN_SET(PIN_LED1); }
    else   { PIN_CLR(PIN_RELAY1); PIN_CLR(PIN_LED1); }
    /* NVS write intentionally omitted -- relay state not persisted on toggle.
     * Flash is rated 10K cycles; saving on every toggle would wear it out.
     * State is restored by master on re-registration after power loss. */
}

static void set_relay2(bool s) {
    relay2_state = s;
    if (s) { PIN_SET(PIN_RELAY2); PIN_SET(PIN_LED2); }
    else   { PIN_CLR(PIN_RELAY2); PIN_CLR(PIN_LED2); }
    /* NVS write intentionally omitted -- see set_relay1 */
}
...
static void self_unregister(void) {
    ...
    PIN_CLR(PIN_LED1);   PIN_CLR(PIN_LED2);
    wipe_registration();
    slot_address      = ADDR_UNASSIGNED;
    mode              = MODE_UNREGISTERED;
    poll_received     = false;
...
void setup() {
    ...
    if (relay1_state) { PIN_SET(PIN_RELAY1); PIN_SET(PIN_LED1); }
    if (relay2_state) { PIN_SET(PIN_RELAY2); PIN_SET(PIN_LED2); }
    last_poll_ms = millis();
    startup_blink();
}
...
void loop() {
    ...
    /* Heartbeat LED3:
     * Registered   -- brief 250ms pulse every 5s
     * Unregistered -- rapid 200ms blink            */
    if (mode == MODE_REGISTERED) {
        if (hb && (millis()-last_hb) >= 250) {
            last_hb=millis(); hb=false; PIN_CLR(PIN_LED3);
        } else if (!hb && (millis()-last_hb) >= 4750) {
            last_hb=millis(); hb=true;  PIN_SET(PIN_LED3);
        }
    } else {
        if ((millis()-last_hb) >= 200) {
            last_hb=millis(); hb=!hb;
            if (hb) PIN_SET(PIN_LED3); else PIN_CLR(PIN_LED3);
        }
    }
}

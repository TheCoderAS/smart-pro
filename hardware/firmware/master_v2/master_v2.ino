...
<div class="card">
  <h2>Mesh Network</h2>
  <p>Join an existing mesh by entering the PIN shown on any master
     already in the mesh. Or leave the current mesh.</p>
  <div id="mesh-status" style="margin-bottom:12px;font-size:12px;color:#666">
    Loading...
  </div>
  <div id="mesh-join-section">
    <input id="mesh-pin-input" type="text" inputmode="numeric"
           maxlength="6" placeholder="6-digit PIN"
           style="width:100%;padding:11px;background:#111128;
                  border:1px solid #2a2a4a;border-radius:10px;
                  color:#eee;font-size:18px;letter-spacing:6px;
                  text-align:center;margin-bottom:10px;outline:none;
                  font-family:monospace"/>
    <button class="btn" id="join-btn" onclick="joinMesh()"
            style="margin-bottom:8px">Join Mesh</button>
  </div>
  <button class="btn" id="leave-btn" onclick="leaveMesh()"
          style="display:none;background:#2a0a0a;color:#ff4e4e;
                 border:1px solid #ff4e4e">Leave Mesh</button>
...
function loadMesh(){
  ...
  xhr.onload=function(){
    ...
    if(d.active){
      ms.innerHTML='<span style="color:#4eff4e">In mesh - '+
        d.peer_count+' peer'+(d.peer_count!==1?'s':'')+' online</span>';
...
function joinMesh(){
  var pin=document.getElementById('mesh-pin-input').value.trim();
  if(pin.length!==6){
  ...
  xhr.onload=function(){
    if(xhr.status===200){
      ...
      setTimeout(function(){ loadMesh(); },3000);
    } else {
      document.getElementById('mesh-msg').innerHTML=
        '<span class="err">Failed: '+xhr.responseText+'</span>';
      document.getElementById('join-btn').disabled=false;
      document.getElementById('join-btn').textContent='Join Mesh';
    }
  };
  xhr.send();
}
...
function leaveMesh(){
  if(!confirm('Leave the mesh? This master will operate independently.')) return;
  var xhr=new XMLHttpRequest();
  xhr.open('POST','/api/mesh/leave',true);
  xhr.onload=function(){
    document.getElementById('mesh-msg').innerHTML=
      '<span class="ok">Left mesh.</span>';
    loadMesh();
  };
  xhr.send();
}
...
.top-btn.active,.top-btn:hover{border-color:#00d4ff;color:#00d4ff}
.top-btn.scan.scanning{border-color:#ffd700;color:#ffd700;animation:pulse 1s infinite}
@keyframes pulse{0%,100%{opacity:1}50%{opacity:0.5}}
@keyframes spin{to{transform:rotate(360deg)}}

/* Accordion */
.master-section{background:#1a1a2e;border-radius:14px;
                margin-bottom:12px;border:1px solid #2a2a4a;
                overflow:hidden;transition:opacity 0.3s}
.master-section.offline{opacity:0.4}
.master-header{display:flex;align-items:center;justify-content:space-between;
               padding:14px 16px;cursor:pointer;user-select:none}
.master-header:hover{background:#1e1e38}
.master-left{display:flex;align-items:center;gap:10px}
.master-arrow{color:#555;font-size:12px;transition:transform 0.2s;width:16px}
.master-arrow.open{transform:rotate(90deg);color:#00d4ff}
.master-name-wrap{display:flex;flex-direction:column}
.master-name{font-size:15px;font-weight:600}
.master-sub{font-size:10px;color:#555;margin-top:1px}
.master-badge{font-size:10px;padding:3px 8px;border-radius:12px;font-weight:700}
...
.badge-self{background:#0a2a3a;color:#00d4ff;border:1px solid #00d4ff}

/* Switch grid inside accordion */
.master-body{display:none;padding:0 12px 12px}
.master-body.open{display:block}
.sw-grid{display:grid;grid-template-columns:1fr 1fr;gap:10px}
.sw-card{background:#111128;border-radius:12px;padding:14px;
         border:1px solid #1e1e38;border-left:4px solid #444;
         position:relative;transition:opacity 0.3s}
.sw-card.offline{opacity:0.4}
.sw-card.reorder-mode{border-style:dashed}
.sw-top{display:flex;align-items:flex-start;justify-content:space-between;
        margin-bottom:10px}
.sw-name{font-size:13px;font-weight:600;line-height:1.3;flex:1}
.sw-menu-btn{background:none;border:none;color:#555;font-size:15px;
             cursor:pointer;padding:2px 4px;line-height:1}
.sw-menu-btn:hover{color:#eee}
.sw-dropdown{position:absolute;right:8px;top:34px;background:#1e1e38;
             border:1px solid #2a2a4a;border-radius:10px;min-width:130px;
...
.toggle{width:100%;padding:10px;border:none;border-radius:8px;
        font-size:13px;font-weight:700;cursor:pointer;transition:all 0.15s;
        position:relative}
.toggle.on{background:#00d4ff;color:#000;box-shadow:0 0 8px rgba(0,212,255,0.2)}
.toggle.off{background:#0d0d20;color:#555;border:1px solid #2a2a4a}
.toggle:active{transform:scale(0.96)}
.toggle:disabled{cursor:not-allowed}
.toggle .spinner{display:none;width:13px;height:13px;border:2px solid rgba(0,0,0,0.3);
...
.move-btns{display:grid;grid-template-columns:1fr 1fr 1fr;
           grid-template-rows:1fr 1fr 1fr;gap:3px;margin-top:8px}
.move-btn{background:#1e1e38;border:1px solid #2a2a4a;color:#888;
          padding:5px;border-radius:5px;font-size:11px;cursor:pointer;text-align:center}
.move-btn:hover{background:#2a2a4a;color:#eee}
.move-btn.blank{background:none;border:none;pointer-events:none}
.empty{color:#444;font-size:13px;text-align:center;padding:24px;
       grid-column:1/-1}
/* Boot overlay */
...
.ext-item{background:#111128;border-radius:12px;padding:14px;
...
.btn{padding:9px 14px;border:none;border-radius:8px;
...
/* Mesh PIN modal */
.pin-display{font-size:36px;font-weight:700;letter-spacing:8px;
             color:#00d4ff;text-align:center;padding:16px 0;
             font-family:monospace}
.pin-timer{font-size:12px;color:#555;text-align:center;margin-top:4px}
</style>
</head>
<body>
...
<!-- Mesh PIN modal (invite) -->
<div class="overlay" id="pin-overlay">
  <div class="input-modal">
    <h2>Invite New Master</h2>
    <p style="color:#666;font-size:12px;margin-bottom:12px">
      On the new master, go to Settings > Mesh > Join Mesh and enter this PIN.
    </p>
    <div class="pin-display" id="pin-display">------</div>
    <div class="pin-timer" id="pin-timer">Valid for 60 seconds</div>
    <div class="btn-row" style="margin-top:16px">
      <button class="btn ghost" onclick="closePinModal()">Close</button>
    </div>
  </div>
</div>
...
var allData={};
var reorderMode=false, reorderList=[], reorderMasterId=null;
var renameTarget=null, renameType=null;
var pendingToggles={};
var expandedMaster=null;  /* which master accordion is open */
var pinTimer=null;
...
/* ?? Build switch card HTML ?? */
function switchCard(sw, idx, isMesh, masterUid){
  var off=!sw.online;
  var menuHtml='';
  if(!reorderMode){
    menuHtml='<button class="sw-menu-btn" onclick="toggleSwMenu(\''+sw.id+'_'+masterUid+'\',event)">&#8942;</button>'+
      '<div class="sw-dropdown" id="menu-'+sw.id+'_'+masterUid+'">'+
        '<div class="sw-dropdown-item" onclick="renameSwitch(\''+sw.id+'\',\''+
          sw.name.replace(/'/g,"\\'")+'\''+
          (isMesh?(',\''+masterUid+'\''):'')+')">Rename</div>'+
      '</div>';
  }
  var btnId='btn-'+sw.id+'_'+masterUid;
  var clickFn=isMesh?
    'toggleMesh(\''+masterUid+'\',\''+sw.id+'\','+sw.ch+')':
    'toggle(\''+sw.id+'\')';
  var moveHtml='';
  if(reorderMode&&!isMesh){
    moveHtml='<div class="move-btns">'+
  ...
  return '<div class="sw-card'+(off?' offline':'')+(reorderMode?' reorder-mode':'')+
...
function render(d){
  ...
  document.getElementById('sub').textContent='192.168.4.1 | '+uptime(d.uptime);
  ...
  /* Confirm pending toggles for local switches */
  (d.switches||[]).forEach(function(sw){
    confirmToggle(sw.id+'_'+d.self_uid, sw.state);
  });

  /* Build accordion sections */
  /* 1. Self (local master) ? always first in accordion */
  var sections=[];

  /* Local master section */
  var localSwitches=reorderMode?reorderList:(d.switches||[]);
  var localHtml='';
  if(localSwitches.length===0){
    localHtml='<div class="empty">No switches yet. Tap Scan.</div>';
  } else {
    localSwitches.forEach(function(sw,idx){
      if(!sw||!sw.id||!sw.name) return;
      localHtml+=switchCard(sw,idx,false,d.self_uid);
    });
  }
  sections.push({
    uid:    d.self_uid,
    name:   d.master_name||'Master 1',
    online: true,
    isSelf: true,
    body:   '<div class="sw-grid">'+localHtml+'</div>'
  });

  /* Mesh peer sections ? sorted alphabetically by name */
  var peers=(d.mesh_peers||[]).filter(function(p){
    var has=false;
    for(var k=0;k<4;k++) if(p.uid&&p.uid!='00000000') has=true;
  ...
  peers.forEach(function(p){
    var pHtml='';
    if(!p.online){
      pHtml='<div class="empty" style="color:#333">Offline</div>';
    } else {
      (p.switches||[]).forEach(function(sw,idx){
        confirmToggle(sw.id+'_'+p.uid, sw.state);
        pHtml+=switchCard(sw,idx,true,p.uid);
      });
    }
    sections.push({
      uid:    p.uid,
      ...
      online: p.online,
      isSelf: false,
      body:   '<div class="sw-grid">'+pHtml+'</div>'
    });
  });

  /* If no expandedMaster set, default to self */
  if(!expandedMaster) expandedMaster=d.self_uid;

  /* Render sections */
  var html='';
  sections.forEach(function(sec){
    var isOpen=(sec.uid===expandedMaster);
    var badge=sec.isSelf?
      '<span class="master-badge badge-self">THIS</span>':
      (sec.online?
        '<span class="master-badge badge-online">ONLINE</span>':
        '<span class="master-badge badge-offline">OFFLINE</span>');
    var swCount=sec.isSelf?
      (d.switches||[]).length:
      ((allData.mesh_peers||[]).find(function(p){return p.uid===sec.uid;})||{switches:[]}).switches.length;
    html+='<div class="master-section'+((!sec.online&&!sec.isSelf)?' offline':'')+'">'+
      '<div class="master-header" onclick="toggleAccordion(\''+sec.uid+'\')">'+
        '<div class="master-left">'+
          '<span class="master-arrow'+(isOpen?' open':'')+'" id="arr-'+sec.uid+'">&#9654;</span>'+
          '<div class="master-name-wrap">'+
            '<span class="master-name">'+sec.name+'</span>'+
            '<span class="master-sub">'+swCount+' switch'+(swCount!==1?'es':'')+'</span>'+
          '</div>'+
        '</div>'+
        badge+
      '</div>'+
      '<div class="master-body'+(isOpen?' open':'')+'\" id="body-'+sec.uid+'">'+
        sec.body+
      '</div>'+
    '</div>';
  });

  /* Add mesh section if mesh active */
  if(d.mesh_active){
    html+='<div style="text-align:center;margin-top:8px">'+
      '<button class="top-btn" onclick="showInvite()" '+
        'style="font-size:11px;padding:6px 12px">+ Invite Master</button>'+
    '</div>';
  } else {
    html+='<div style="text-align:center;margin-top:8px">'+
      '<button class="top-btn" onclick="createMesh()" '+
        'style="font-size:11px;padding:6px 12px;border-color:#ffd700;color:#ffd700">'+
        'Create Mesh</button>'+
    '</div>';
  }
  ...
  /* Pending data */
  var pendingData=d.pending||[];
  var offlineSlots=d.offline_slots||[];
  if(pendingData.length>0&&
     !document.getElementById('batch-overlay').classList.contains('show')){
    renderBatchModal(pendingData, offlineSlots);
...
function toggleAccordion(uid){
  ...
  var prev=expandedMaster;
  expandedMaster=(prev===uid)?null:uid;
...
function toggle(id){
  ...
  var btn=document.getElementById('btn-'+id+'_'+uid);
  ...
  pendingToggles[key]={timer:timer,expected:!isOn};
  var ch=id.split('_')[1];
...
/* ?? Mesh toggle ?? */
function toggleMesh(peerUid, swId, ch){
  var btn=document.getElementById('btn-'+swId+'_'+peerUid);
  if(!btn||btn.disabled) return;
  var isOn=btn.classList.contains('on');
  btn.classList.remove('on','off'); btn.classList.add(isOn?'off':'on','loading');
  btn.querySelector('.lbl').textContent=isOn?'OFF':'ON';
  btn.disabled=true;
  var key=swId+'_'+peerUid;
  var timer=setTimeout(function(){
    btn.classList.remove('loading');
    btn.classList.remove(isOn?'off':'on');
    btn.classList.add(isOn?'on':'off');
  ...
  pendingToggles[key]={timer:timer,expected:!isOn};
  fetch('/api/mesh/relay?peer_uid='+peerUid+'&sw_id='+swId+'&ch='+ch,
...
function confirmToggle(key,actual){
...
async function createMesh(){
  if(!confirm('Create a new mesh network with this master as the first member?')) return;
...
async function showInvite(){
  ...
  pinTimer=setInterval(function(){
    remaining--;
    document.getElementById('pin-timer').textContent='Valid for '+remaining+'s';
    if(remaining<=0){
...
function toggleReorder(){
  ...
  reorderMasterId=allData.self_uid;
...
function swap(arr,a,b){
  ...
  var t=arr[a]; arr[a]=arr[b]; arr[b]=t; return true;
}
function moveUp(idx){if(swap(reorderList,idx,idx-2))render(allData);}
function moveDown(idx){if(swap(reorderList,idx,idx+2))render(allData);}
function moveLeft(idx){if(swap(reorderList,idx,idx-1))render(allData);}
function moveRight(idx){if(swap(reorderList,idx,idx+1))render(allData);}
...
/* ?? Menus ?? */
function toggleSwMenu(key,e){
  e.stopPropagation();
  var d=document.getElementById('menu-'+key);
...
function renameSwitch(id,current,peerUid){
  document.querySelectorAll('.sw-dropdown.show').forEach(function(d){d.classList.remove('show');});
  renameTarget={id:id,peerUid:peerUid||null}; renameType='switch';
...
/* ?? Batch modal ?? */
function renderBatchModal(pendingData, offlineSlots){
  ...
  list.innerHTML=pendingData.map(function(p){
    var opts=offlineSlots.map(function(s){
    ...
    return '<div class="ext-item">'+
      ...
      '<div class="sw-inputs">'+
        '<div><label>Switch 1</label>'+
          '<input id="sw1-'+p.uid+'" type="text" placeholder="e.g. Bed Light" maxlength="23"/></div>'+
        '<div><label>Switch 2</label>'+
          '<input id="sw2-'+p.uid+'" type="text" placeholder="e.g. Bed Fan" maxlength="23"/></div>'+
      '</div>'+
      '<select id="action-'+p.uid+'">'+
        '<option value="new">Add as new</option>'+opts+
      '</select>'+
...
async function assignExt(uid){
  ...
  var action=document.getElementById('action-'+uid).value;
  var slot=-1;
  ...
  if(pd.length===0) closeBatchModal();
  else renderBatchModal(pd, allData.offline_slots||[]);
}
...
function connect(){
  ...
  ws.onmessage=function(e){
    try{
      ...
      console.log('Raw:',e.data.substring(0,100));
    }
  };
  ws.onclose=function(){

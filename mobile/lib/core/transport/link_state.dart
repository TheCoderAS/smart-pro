...
class LinkMonitor extends Notifier<LinkState> {
  ...
  Future<void> _tick() async {
    ...
    try {
      ...
      _degrade();
      // Re-pinning is the whole of the app's self-healing: once Android
      // itself rejoins the Wi-Fi, the next heartbeat lands. There is
      // deliberately no app-driven network JOIN here any more — Android's
      // app-scoped join demands a tap-through system dialog, which can
      // never happen unattended, so the old auto-rejoin was noise at
      // best and a radio hog at worst.
      await _rebindWifi();
    } finally {
      _probing = false;
    }
